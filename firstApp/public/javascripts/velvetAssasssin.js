...
function getComments() {
  $.get("/users/get_current_user", function(data) {
    ...
    $.get("/get_velvetAssassin_Comments", function (data) {
        var posts = "";
...
$("#postForm").submit(function (event) {
    ...
    $.post("/add_velvetAssassin_Comment", {
...
$("#feedPosts").click(function (event) {
    ...
    if (event.target.value == 0) {
      swal("Comment deleted!")
        $.ajax({
            url: '/remove_velvetAssassin_Comment/' + event.target.name,
            type: 'DELETE',
...
$("#feedPosts").click(function (event) {
    ...
    if (event.target.value == 1) {
      ...
      if(updatedComment == "" || updatedComment == null) {
        ...
        $.ajax({
            url: '/update_velvetAssassin_Comment/' + event.target.name,
            type: 'PUT',
